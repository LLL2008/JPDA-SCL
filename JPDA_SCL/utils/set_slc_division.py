...
def set_division(class_num, class_list, image_slice, label_slice, mode, rate, row_col):
    ...
    class_len = torch.full([class_num], 0)   #统计类别数目

    for p, cls in enumerate(class_list):  #enumerate() 函数用于将一个可遍历的数据对象
        a = 0
        for i in range(N):
            if label_slice[i] == cls:
                a = a + 1
                image_idx[p, a - 1] = i    #标识类别所在的位置
                class_len[p] += 1
    ...
    if mode == 'train':  # train
        for i in range(class_num):
            if rate < 1:
                for j in range(0, int(rate * class_len[i])):  #按要求选取相应类别数量
                    a = a + 1
                    image_ex[a - 1] = image_slice[image_idx[i, j]]  #选取数据
                    label_ex[a - 1] = class_list[i]                 #记录对应数据的标签
                    row_col_ex[a - 1] = row_col[image_idx[i, j]]   #记录对应数据在整个图中的位置
            else:
        ...
        for i in range(class_num):
            if rate < 1:
                for j in range(int(rate * class_len[i]), int(rate*2 * class_len[i])): #在选取了train的数据之后选取val数据
                    a = a + 1
                    image_ex[a - 1] = image_slice[image_idx[i, j]]
                    label_ex[a - 1] = class_list[i]
                    row_col_ex[a - 1] = row_col[image_idx[i, j]]
...
def set_division_pro(class_num, class_list, image_slice, label_slice, mode, rate: list):
    ...
    image_idx = torch.full([class_num, N], 0)
    class_len = torch.full([class_num], 0)  #每个类别的数量统计
