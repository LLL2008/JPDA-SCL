...
class MDGTnet(nn.Module):
    def __init__(self, in_ch, out_ch: list, padding, slice_size: int, spec_range: list, class_num: int):
        ...
        self.IFEM = IFEM(out_ch[5], [out_ch[0], out_ch[1],
                         out_ch[2], out_ch[3]], padding=padding)

        self.classifier = nn.Sequential(nn.Flatten(),
                                        nn.Linear(
                                            slice_size*slice_size*out_ch[3], out_ch[6]),
    ...
    def forward(self, x_intra, x_inter):
        ...
        __, __, __, y_inter_4, y_side_1, y_side_2, y_side_3, y_side_4 = self.IFEM(
            y_inter, x_side)
